    ...
    <div class="active-filters-bar" v-if="hasActiveFilters">
      <div class="container">
        <div class="active-filter-tags">
          ...
          <div v-else-if="selectedCategory === null" class="filter-tag">
            <span class="tag-label">카테고리:</span>
            <span class="tag-value">전체</span>
          </div>
...
export default {
  ...
  setup(props) {
    ...
    // beforeRouteUpdate 훅 추가
    onBeforeRouteUpdate((to, from, next) => {
      console.log('Route updating from:', from.fullPath, 'to:', to.fullPath);
      // 라우트 업데이트 시 필터 초기화
      setTimeout(() => {
        initializeFilters();
        fetchCampaigns();
    ...
    const initializeFilters = () => {
      console.log('Initializing filters from route:', {
      ...
      // 카테고리 설정
      if (route.name === 'delivery' && !props.category && !route.params.category) {
        // delivery 기본 라우트인 경우 카테고리를 null로 설정
      ...
      console.log('Filters initialized:', {
        ...
        selectedCampaignType: selectedCampaignType.value,
        selectedChannelType: selectedChannelType.value,
    ...
    // props.category 변경 감시
    watch(() => props.category, (newCategory) => {
      if (newCategory === undefined && route.name === 'delivery') {
        // props.category가 undefined이고 delivery 라우트인 경우 null로 설정
        selectedCategory.value = null;
      } else {
    ...
    watch([selectedCategory, selectedCampaignType, selectedChannelType, sortOption, currentPage], ([cat, camType, chanType, sort, page]) => {
      console.log('Filters changed:', { cat, camType, chanType, sort, page });
      updateUrl();
    ...
    const selectCampaignType = (type) => {
      console.log('Selecting campaign type:', type);
    ...
    const selectChannelType = (channel) => {
      console.log('Selecting channel type:', channel);
